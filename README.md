...
場合は `--runs` で回数を、`--parallel` で並列実行数を指定できます。1つの学習で複数環境を
同時に利用する場合は `--num-envs` を指定してください。環境の描画速度
を調整する `--speed-multiplier` オプションも利用可能です。`train.py` は1エピソード
ごとに鬼と逃げの学習対象を交互に切り替え、エピソード数は `--episodes` で指定でき
ます。
